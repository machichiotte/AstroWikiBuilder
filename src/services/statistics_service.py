...
from src.models.reference import SourceType, DataPoint # SourceType for stats key
    ...
    def generate_statistics(self, exoplanets: List[Exoplanet]) -> Dict[str, Any]:
        ...
        for exoplanet in exoplanets:
            ...
            for field_name in exoplanet.__dataclass_fields__:
                if field_name in ['name', 'other_names']: # name is str, other_names is List[str]
                    continue
            ...
            if exoplanet.discovery_date and exoplanet.discovery_date.value:
                try:
                    ...
                    if hasattr(year_val, 'year'): # if it's a datetime object
                        year = year_val.year
                    elif isinstance(year_val, str) and len(year_val) >= 4 and year_val[:4].isdigit():
                        year = int(year_val[:4])
                    elif isinstance(year_val, (int, float)):
                        year = int(year_val)
                    else: # Skip if not easily parsable as a year
                        logger.debug(f"Could not parse year from discovery_date.value: {year_val}")
                        continue
                    stats['discovery_years'][year] = stats['discovery_years'].get(year, 0) + 1
                except ValueError:
                    logger.warning(f"Could not parse year from discovery_date.value: {exoplanet.discovery_date.value} for exoplanet {exoplanet.name}")
