...
from src.utils.wikipedia_checker import WikipediaChecker, WikiArticleInfo # Assuming WikiArticleInfo is also in wikipedia_checker
...
class WikipediaService:
    ...
    def get_all_articles_info_for_exoplanets(self, exoplanets: List[Exoplanet]) -> Dict[str, Dict[str, WikiArticleInfo]]:
        """
        ...
        for exoplanet in exoplanets:
            ...
            for title_to_check in titles_for_this_exoplanet:
                if title_to_check not in titles_to_batch_check: # Avoid duplicate checks if same alias used by multiple planets (unlikely but possible)
        ...
        checked_article_infos: Dict[str, WikiArticleInfo] = {} # title_queried -> WikiArticleInfo

        for i in range(0, total_titles_to_check, batch_size):
            batch_titles = titles_to_batch_check[i:i + batch_size]
            logger.info(f"Checking Wikipedia batch: {i//batch_size + 1}/{(total_titles_to_check + batch_size - 1)//batch_size} ({len(batch_titles)} titles)")
            
            # Prepare specific context for this batch
            batch_context = {
                title: {"host_star_name": context_for_titles[title]['host_star_name']} 
                for title in batch_titles if title in context_for_titles
            }

            try:
                # `check_multiple_articles` returns Dict[queried_title, WikiArticleInfo]
                batch_wiki_infos = self.checker.check_multiple_articles(batch_titles, exoplanet_context=batch_context)
                ...
                for title in batch_titles: # Mark all in batch as error
                     checked_article_infos[title] = WikiArticleInfo(exists=False, title=title, queried_title=title, url=f"Error: {e}")
        ...
        for queried_title, wiki_info in checked_article_infos.items():
            if queried_title in context_for_titles:
                ...
                if exoplanet_name_origin in all_results:
                    ...
                    logger.warning(f"Exoplanet name '{exoplanet_name_origin}' from context not found in initial results for title '{queried_title}'.")
            else:
                logger.warning(f"Queried title '{queried_title}' from Wikipedia check not found in context map.")
    ...
    def separate_articles_by_status(
        self,
        all_exoplanet_articles_info: Dict[str, Dict[str, WikiArticleInfo]]
        ...
        for exoplanet_name, articles_for_exo in all_exoplanet_articles_info.items():
            has_any_existing_article = any(info.exists for info in articles_for_exo.values())
        ...
        logger.info(f"Separation complete. Existing: {len(existing_articles_map)}, Missing: {len(missing_articles_map)}")
        return existing_articles_map, missing_articles_map

    def format_wiki_links_data_for_export(
        self,
        exoplanets: List[Exoplanet], # Need full exoplanet objects for context like host_star
        ...
        """
        logger.info(f"Formatting Wikipedia link data for {len(exoplanet_articles_info)} exoplanets...")
        formatted_data = []
        ...
        for exoplanet_name, articles in exoplanet_articles_info.items():
            ...
            if not exoplanet_obj:
                logger.warning(f"Exoplanet object for '{exoplanet_name}' not found during formatting. Skipping.")
                continue
            ...
            for queried_name, info in articles.items(): # queried_name is the name/alias that was checked
                record = {
                    ...
                    'wikipedia_title': info.title if info.exists else None, # Actual title on Wikipedia
                    'is_redirect': info.is_redirect if info.exists else None,
                    'redirect_target': info.redirect_target if info.exists and info.is_redirect else None,
                    'url': info.url if info.exists else None,
                    'host_star': exoplanet_obj.host_star.value if exoplanet_obj.host_star else None
                }
