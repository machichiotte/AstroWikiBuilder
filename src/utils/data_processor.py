...
class DataProcessor:
    ...
    def export_wikipedia_links_data(self,
                                    ...
                                    status_description_for_filename: str) -> None:
        """
        ...
        """
        logger.info(f"Preparing to export Wikipedia links data for status: {status_description_for_filename}")
        all_exoplanets_from_repo = self.repository.get_all_exoplanets() # Still needed for context in format_wiki_links_data_for_export
        ...
        if not formatted_list:
            logger.info(f"No formatted data to export for status '{status_description_for_filename}'.")
            return

        csv_filename = f"{filename_base}_{status_description_for_filename}_wiki_links.csv"
        json_filename = f"{filename_base}_{status_description_for_filename}_wiki_links.json"
        ...
        logger.info(f"Wikipedia links data for '{status_description_for_filename}' exported to {csv_filename} and {json_filename}")
