...
class StarUtils:
    ...
    def get_star_description(self, exoplanet: Exoplanet) -> str:
        ...
        spectral = (
            exoplanet.spectral_type.value[0]
        ...
        if (
            exoplanet.apparent_magnitude
            and exoplanet.apparent_magnitude.value is not None
        ):
            mag = self.format_utils.format_numeric_value(
                exoplanet.apparent_magnitude.value
            )
    ...
    def get_star_characteristics(self, exoplanet: Exoplanet) -> Dict[str, str]:
        ...
        if (
            ...
            chars["Magnitude apparente"] = self.format_utils.format_numeric_value(
                exoplanet.apparent_magnitude.value
            )
        if exoplanet.distance and exoplanet.distance.value is not None:
            ly = self.format_utils.parsecs_to_lightyears(exoplanet.distance.value)
            if ly is not None:
                chars["Distance"] = (
                    f"{self.format_utils.format_numeric_value(ly)} années‑lumière"
                )
        if exoplanet.constellation and exoplanet.constellation.value:
    ...
    def get_constellation_formatted(self, exoplanet: Exoplanet) -> str:
        ...
        constellation_name = self.get_constellation(
            exoplanet
