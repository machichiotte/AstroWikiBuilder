...
class ExoplanetInfoboxGenerator:
    ...
    def _process_field(self, exoplanet: DataSourceExoplanet, mapping: FieldMapping) -> str:
        ...
        # 3.a Extract raw value and unit, then format if valid
        ...
        if self._is_valid_value(value):
            ...
            if mapping.formatter:
                ...
                value_part = self._format_by_type(
                    value, unit, mapping.infobox_field, mapping.field_type, mapping.default_unit
                )

        if value_part:
            infobox_block_parts.append(value_part)
            # Only if value_part is valid, attempt to add notes
            # TODO ici on verifie que le champ notes existe dans exoplanet_notes.md

            print("_process_field mapping", mapping)
            print("_process_field field", mapping.infobox_field)
            print("_process_field condition", mapping.condition)
            if self._is_valid_notes(mapping.infobox_field):
                notes_ref = self._extract_notes(datapoint, exoplanet)
                if notes_ref:
                    notes_line = f"| {mapping.infobox_field} notes = {notes_ref}"
                    infobox_block_parts.append(notes_line)

        # If value_part is empty, infobox_block_parts will be empty, and nothing will be returned for this field.
        return "\n".join(infobox_block_parts)

    def _is_valid_notes(self, field: str) -> bool:
        notes_fields = [
            "époque étoile",
            "ascension droite",
            "déclinaison",
        ...
        if field.lower() in notes_fields:
            return True
        return False
    ...
    def _extract_notes(
        ...
        if exoplanet.name and hasattr(exoplanet.name, 'value') and exoplanet.name.value is not None:
            ...
            # Fallback to string of DataPoint obj itself
            current_exoplanet_name_str = str(exoplanet.name)
        ...
        try:
            ...
            full_ref_wikitext = reference_obj.to_wiki_ref(
                current_exoplanet_name_str)
        ...
        if not isinstance(source_name_str, str):
            ...
            source_name_str = str(source_name_str)
        ...
        ref_tag = self.reference_manager.add_reference(
            source_name_str, full_ref_wikitext)
        return ref_tag
