...
def main():
    ...
    sirius_data = {
        ...
        "designations": DataPoint(["Alpha Canis Majoris", "α CMa", "HD 48915", "HR 2491"])
    }
    ...
    proxima_data = {
        ...
        "designations": DataPoint("V645 Cen, Alpha Centauri C, HIP 70890") # String, Star.__post_init__ handles it
    }
    ...
    sun_data = {
        ...
        "distance": DataPoint(value=1.0, unit="AU (distance from Earth)"), # Illustrative, not standard infobox distance
    ...
    for star_obj in sample_stars:
        star_name = star_obj.name.value if star_obj.name and hasattr(star_obj.name, 'value') else "UnknownStar"
    ...
    if star_drafts_to_save:
        ...
        full_drafts_path = os.path.join(drafts_dir, "missing")
        logger.info(f"Star draft generation complete. Drafts saved in '{os.path.abspath(full_drafts_path)}'.")
    else:
