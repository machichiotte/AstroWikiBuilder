...
class MockDataPoint:
    def __init__(
        ...
        reference: Optional[MockReference] = None,
    ):
        self.value = value
        self.unit = unit
...
class TestWikipediaGenerator(unittest.TestCase):
    ...
    def test_date_formatting_in_article(self):
        ...
        exoplanet.discovery_date = MockDataPoint(
            value="2013-05-20"
    ...
    def test_infobox_default_units(self):
        ...
        exoplanet.mass = MockDataPoint(
            value="1", unit="M_J"
        )  # M_J is default for "masse"
        exoplanet.radius = MockDataPoint(
            value="1", unit="R_J"
        )  # R_J is default for "rayon"

        # Case 2: Unit is different from default
        exoplanet.temperature = MockDataPoint(
            value="1200", unit="°C"
        )  # Default for "température" is K
        ...
        exoplanet.eccentricity = MockDataPoint(
            value="0.1", unit="n/a"
        )  # Eccentricity has no default in map
        ...
        self.assertNotIn(
            "| distance unité =", infobox
        )  # No unit in data, so no unit line
        self.assertIn(
            "| eccentricité unité = n/a", infobox
        )  # No default, so unit is shown
    ...
    def test_first_reference_formatting_globally(self):
        ...
        mock_ref_nasa = MockReference(
            source_type_val=MockSourceType.NEA, content_template=ref_nasa_content
        )

        ref_epe_content = "{{Lien web | titre=EPE1 | url=http://epe.eu/1 }}"
        mock_ref_epe = MockReference(
            source_type_val=MockSourceType.EPE, content_template=ref_epe_content
        )
        ...
        exoplanet.orbital_period = MockDataPoint(
            value="365", unit="j", reference=mock_ref_epe
        )
        ...
        expected_nasa_full = (
            f'<ref name="{MockSourceType.NEA}">{ref_nasa_content}</ref>'
        )
        expected_nasa_short = f'<ref name="{MockSourceType.NEA}" />'

        self.assertEqual(
            article.count(expected_nasa_full), 1, "NASA full ref should appear once"
        )
        self.assertTrue(
            article.count(expected_nasa_short) >= 1,
            "NASA short ref should appear at least once",
        )
        ...
        self.assertEqual(
            article.count(expected_epe_full), 1, "EPE full ref should appear once"
        )
    ...
    def test_conditional_notes_subsection(self):
        ...
        ref_regular_content = (
            "{{Lien web | titre=Regular | url=http://example.com/regular }}"
        )
        mock_ref_regular = MockReference(
            source_type_val=MockSourceType.CUSTOM, content_template=ref_regular_content
        )
        exoplanet.temperature = MockDataPoint(
            value="300", unit="K", reference=mock_ref_regular
        )
        ...
        self.assertIn(
            "=== Références ===", article_no_notes
        )  # Main refs should still be there
        ...
        exoplanet.eccentricity = MockDataPoint(
            value="0.1", reference=mock_ref_grouped_note
        )
        ...
        self.assertIn(
            mock_ref_grouped_note.to_wiki_ref(), article_with_notes
        )  # Ensure the note itself is in the article
    ...
    def test_distance_formatting_lightyears_parsecs(self):
        ...
        mock_ref_dist = MockReference(
            source_type_val=MockSourceType.NEA, content_template=ref_dist_content
        )
        exoplanet.distance = MockDataPoint(
            value=pc_val, unit="pc", reference=mock_ref_dist
        )

        article = self.generator.generate_article_content(exoplanet)

        ly_val_expected = math.floor(
            pc_val * 3.26156
        )  # Using floor to match precision=0 for positive numbers
        ...
        expected_ly_str = self.generator._format_numeric_value(
            pc_val * 3.26156, precision=0
        )

        # Formatted PC value (number only, from _format_numeric_value)
        expected_pc_num_str = self.generator._format_numeric_value(
            pc_val, precision=2
        )  # Default precision for pc
